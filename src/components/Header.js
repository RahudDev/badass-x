    ...
    <>
            ...
            <Link className="nav-link text-white" onClick={(e) => HandlePageClick(e, isLoggedIn ? "/dashboard" : "/home", contactSectionRef, navigate)}><FontAwesomeIcon icon={isLoggedIn ? faTachometerAlt : faHome} className="me-2" />{isLoggedIn ? "Dashboard" : t('home')}</Link>
              ...
              <li className="nav-item dropdown">
                ...
                <FontAwesomeIcon icon={faUsers} className="me-2" />Affiliates
                  ...
                  <Dropdown.Menu>
                    <Dropdown.Item onClick={(e) => HandlePageClick(e, '/referral-link', contactSectionRef, navigate)}><strong><FontAwesomeIcon icon={faHandshake}/> Affiliates Program</strong></Dropdown.Item>
                    <Dropdown.Item onClick={(e) => HandlePageClick(e, '/referral-stats', contactSectionRef, navigate)}><strong>  <FontAwesomeIcon icon={faChartBar} /> Your Affiliate Stats</strong></Dropdown.Item>
      ...
      <div className={`slide-menu ${isMenuOpen ? 'open' : ''}`}>
        ...
        <ul className="slide-menu-list">
          <li><Link onClick={(e) => {HandlePageClick(e, isLoggedIn ? '/dashboard' : '/home',contactSectionRef, navigate); closeMenu();}}><strong> <FontAwesomeIcon icon={isLoggedIn ? faTachometerAlt : faHome} className="me-2" /> { isLoggedIn ? 'Dashboard' : t('home')}</strong></Link></li>
          <li><Link onClick={(e) => {HandlePageClick(e, isLoggedIn ? '/profile' : '/tasks',contactSectionRef, navigate); closeMenu();}}><strong> <FontAwesomeIcon icon={isLoggedIn ? faUser : faListCheck} className="me-2" /> { isLoggedIn ? 'Your Profile' : t('tasks')}</strong></Link></li>
          <li><Link onClick={(e) => {HandlePageClick(e,  isLoggedIn ? '/history' : '/about',contactSectionRef, navigate); closeMenu();}}><strong> <FontAwesomeIcon icon={isLoggedIn ? faClock : faCircleInfo} className="me-2" /> { isLoggedIn ? 'Your History' : t('about_us')}</strong></Link></li>
          {isLoggedIn && (
            <>
            <li><Link onClick={(e) => {HandlePageClick(e, '/line-chart',contactSectionRef, navigate); closeMenu();}}><strong><FontAwesomeIcon icon={faChartLine} className="me-2" /> Insight</strong></Link></li>
              <li><Link onClick={(e) => {HandlePageClick(e, '/referral-link',contactSectionRef, navigate); closeMenu();}}><strong><FontAwesomeIcon icon={faHandshake} className="me-2" /> Affiliate Program</strong></Link></li>
              <li><Link onClick={(e) => {HandlePageClick(e, '/referral-stats',contactSectionRef, navigate); closeMenu();}}><strong> <FontAwesomeIcon icon={faChartBar} className="me-2" /> Your Affiliate Stats</strong></Link></li>
