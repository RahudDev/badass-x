...
import { App as CapacitorApp } from '@capacitor/app'; // Using an alias to avoid conflicts
...
const App = () => {
  ...
  useEffect(() => {
    const handleDeepLink = (event) => {
        ...
        if (url.hostname === 'freecuan.site') {
            // Handle the deep link accordingly
            console.log('Deep link opened:', url);
            // You can navigate to a specific page or perform an action based on the URL
        }
    };
  ...
  const fetchIp = async () => {
    try {
      ...
      return ipResponse.data.ip; // Return the fetched IP
    } catch (err) {
      console.error('Error fetching user IP:', err);
      return ''; // Fallback to empty string if IP can't be fetched
    }
  };
